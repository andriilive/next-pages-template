import { Button, Kbd, Link as LinkUI, Input, Navbar as NextUINavbar, NavbarContent, NavbarMenu, NavbarMenuToggle, NavbarBrand, NavbarItem, NavbarMenuItem } from '@nextui-org/react'
	...
	return (
		<NextUINavbar maxWidth='xl' position='sticky'>
				...
				<div className='ml-2 hidden justify-start gap-4 lg:flex'>
					{webConfig.navItems.map((item) => (
						<NavbarItem key={item.href}>
							<Link className={clsx(linkStyles({ color: 'foreground' }), 'data-[active=true]:font-medium data-[active=true]:text-primary')} color='foreground' href={item.href}>
								{item.label}
							</Link>
						</NavbarItem>
					))}
			...
			<NavbarContent className='flex w-full gap-3 sm:hidden' justify='end'>
				<LinkUI isExternal href={webConfig.links.github}>
			...
			<NavbarContent className='basis-1/5 sm:basis-full' justify='end'>
				<NavbarItem className='hidden gap-2 sm:flex'>
					<LinkUI isExternal href={webConfig.links.twitter}>
						<TwitterIcon className='text-default-500' />
					</LinkUI>
					<LinkUI isExternal href={webConfig.links.discord}>
				...
				<div className='mx-4 mt-2 flex flex-col gap-2'>
					{webConfig.navMenuItems.map((item, index) => (
						<NavbarMenuItem key={`${item}-${index}`}>
							<LinkUI color={index === 2 ? 'primary' : index === webConfig.navMenuItems.length - 1 ? 'danger' : 'foreground'} href='#' size='lg'>
								{item.label}
							</LinkUI>
						</NavbarMenuItem>
					))}
